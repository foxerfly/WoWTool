...
class WoWConfig:
    ...
    def getaddonV2(self, doc):
        ...
        #      if lastedversion['gameVersionFlavor'] == 'wow_classic'
        #           and int(lastedversion['projectFileId']) > projectFileId:
        #          projectFileId = lastedversion['projectFileId']
        #          pass
        #      pass

        for lastedfile in doc['latestFiles']:
            if not str(lastedfile['fileName']).find('nolib') >= 0 \
               and lastedfile['gameVersionFlavor'] == 'wow_classic' \
               and int(lastedfile['id']) > version:
    ...
    def checkNeedUpdate(self, addonobject):
        ...
        for a in self.saved_list:
            # log(str(a))
            if int(a['id']) == int(addonobject['id']) \
               and int(a['version']) == int(addonobject['version']):
                # log(json.dumps(a))
                res = False
                break
            else:
...
def un_zip(file_name, pathname):
    ...
    for names in zip_file.namelist():
        zip_file.extract(names, pathname)
